...
export const Sponsors: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const response = await generateSponsorResponse(form.company, form.message);
        ...
        <div>
           {!submitted ? (
             ...
             <div className="bg-neon-blue/10 border border-neon-blue/30 p-8 rounded-2xl text-center h-full flex flex-col items-center justify-center backdrop-blur-md">
               ...
               {aiResponse && (
                 <div className="bg-black/50 p-4 rounded-lg max-w-sm text-sm text-left border-l-4 border-neon-purple">
                   <p className="text-xs text-neon-purple mb-1 font-bold">AUTOMATED ACKNOWLEDGEMENT:</p>
                   "{aiResponse}"
                 </div>
               )}
