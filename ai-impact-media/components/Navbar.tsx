...
import { Menu, X, MonitorPlay, ChevronRight, Search, Bell } from 'lucide-react';
      ...
      <div className="max-w-7xl mx-auto px-4 md:px-8">
        <div className="flex items-center justify-between h-14 md:h-16">
          ...
          <div className="flex items-center gap-4">
             {/* Mobile Hamburger (Left aligned like Netflix) */}
          ...
          <div className="flex items-center gap-4 md:gap-6">
             ...
             {/* Admin Link on Desktop only, accessible via mobile menu for mobile */}
             <Link to={AppRoute.ADMIN} className="hidden md:block w-8 h-8 rounded bg-gradient-to-br from-neon-blue to-neon-purple"></Link>
          </div>
        </div>
      </div>
        ...
        <div className={`absolute top-0 bottom-0 left-0 w-[70%] bg-black border-r border-gray-800 transform transition-transform duration-300 ease-out ${isOpen ? 'translate-x-0' : '-translate-x-full'}`}>
           <div className="flex flex-col p-6 space-y-6 mt-14">
              ...
              <div className="border-t border-gray-800 pt-6 mt-4">
                 ...
                 <button className="text-sm text-gray-500 block">Sign Out</button>
              </div>
           </div>
        </div>
      </div>
